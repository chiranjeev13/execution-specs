...
def test_happy_path_self_paid(
    ...
    state_test(
        ...
        tx=tx,
        post={
            sender: Account(code=sender_code, nonce=2),
...
def test_sponsored_transaction(
    ...
    sender = pre.deploy_contract(code=sender_code, balance=10**18)
    sponsor = pre.deploy_contract(code=sponsor_code, balance=10**18)
    ...
    state_test(
        ...
        tx=tx,
        post={
            sender: Account(code=sender_code, nonce=2, balance=10**18),
...
def test_verify_frame_must_approve(
    ...
    sender = pre.deploy_contract(code=verify_code, balance=10**18)
...
def test_verify_frame_oog_invalid(
    ...
    sender = pre.deploy_contract(code=oog_code, balance=10**18)
...
def test_sponsored_balance_and_nonce(
    ...
    state_test(
        ...
        tx=tx,
        post={
            # Sender nonce increments once from deployment nonce 1.
            sender: Account(
                nonce=2,
                balance=initial,
            ),
            # Sponsor pays gas but nonce does NOT change from 1.
            sponsor: Account(
                nonce=1,
            ),
        },
...
def test_payment_revert_atomicity(
    ...
    bad_sponsor = pre.deploy_contract(
        code=Op.REVERT(0, 0),
        balance=initial,
    )
    good_sponsor = pre.deploy_contract(
        code=approve_bytecode(Spec.APPROVE_PAYMENT),
        balance=initial,
    )
