...
class TransactionException(ExceptionBase):
    ...
    """
    Transaction is type 6, but frame list is malformed (wrong count,
    invalid mode, bad RLP, invalid field lengths, etc.).
    """
    ...
    """
    Transaction is type 6, but a frame failed execution
    (VERIFY without APPROVE, exceptional halt, OOG, etc.).
    """
